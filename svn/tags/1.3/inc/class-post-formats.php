...
class BenjaminPostFormat
{
    ...
    public static function init($screens = array())
    {
        ...
        self::$formats = json_decode(BENJAMIN_POST_FORMATS);

        $forms = array_intersect(self::$formats, self::$supported);

        add_action('add_meta_boxes', array('BenjaminPostFormat', 'register_meta'));
        ...
        foreach ($forms as $format) {
            ...
            if (class_exists('BenjaminPostFormat' . ucfirst($format))) {
                $class_name = 'BenjaminPostFormat' . ucfirst($format);
                $class      = new $class_name(self::$screens);
            }
        }

        add_action('save_post', array('BenjaminPostFormat', 'save'));
        add_action('init', array('BenjaminPostFormat', 'cpt_support'), 11);
        add_action('admin_enqueue_scripts', array('BenjaminPostFormat', 'enqueue'));
        add_action('edit_form_after_title', array('BenjaminPostFormat', 'display_meta_boxes'));
    }
